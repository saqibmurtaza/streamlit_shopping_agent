...
os.environ["GOOGLE_APPLICATIONS_CREDENTIALS"] = os.getenv("GOOGLE_APPLICATIONS_CREDENTIALS")
...
from streamlit_shopping_agent.models import ChatMessage
from streamlit_shopping_agent.tools import search_products
from config_agents import shopping_manager, config
...
if user_input:
    st.session_state.chat_history.append(ChatMessage(role="user", content=user_input))
    with st.spinner("Thinking..."):
        response = shopping_manager.run_sync(user_input, run_config=config)
        st.session_state.chat_history.append(ChatMessage(role="assistant", content=response.final_output))
